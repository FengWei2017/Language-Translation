...
%type<str>  NAME
%type<val>  funcdef classdef simple_stmt small_stmt stmt compound_stmt if_stmt while_stmt for_stmt  assert_stmt plus_except 
%type<val>  suite plus_stmt with_stmt try_stmt star_ELIF star_SEMI_small_stmt decorated decorators decorator except_clause
...
file_input // Used in: start
	...
	{   if(mymap.size()) std::cout<<filename<<std::endl;
...
decorator // Used in: decorators
	: AT dotted_name LPAR opt_arglist RPAR NEWLINE {$$=0;}
	| AT dotted_name NEWLINE {$$=0;}
	;
...
decorators // Used in: decorators, decorated
	: decorators decorator {$$=$1+$2;}
	| decorator  {$$=$1;}
	;
decorated // Used in: compound_stmt
	: decorators classdef {$$=$1+$2;}
	| decorators funcdef  {$$=$1+$2;}
	;
funcdef // Used in: decorated, compound_stmt
	: DEF NAME parameters COLON suite 
	{ 
	  $$=1+$5;
	  std::string funcout,funcname;
	  if(inclass) {classfun.push_back($$);   funcout="    M"; funcname=classname+"."+$2; methodnumber++; }
	  else {funcout="    F"; funcname=$2;}
	  funcout=funcout+" "+std::to_string(@1.first_line)+":"+std::to_string((@1.first_column-1));
	  funcout=funcout+" "+ funcname;
	  if($$<6)funcout=funcout+" - A ";
	  else if($$<10)funcout=funcout+" - B ";
	  else funcout=funcout+" - C ";
	  mymap.insert (std::pair<int,std::string>($$,funcout) );
	}
	;
...
simple_stmt // Used in: stmt, suite
	: small_stmt star_SEMI_small_stmt SEMI NEWLINE {$$=$1+$2;}
	| small_stmt star_SEMI_small_stmt NEWLINE {$$=$1+$2;}
	;
star_SEMI_small_stmt // Used in: simple_stmt, star_SEMI_small_stmt
	: star_SEMI_small_stmt SEMI small_stmt {$$=$1+$3;}
	| %empty {$$=0;}
	;
...
if_stmt // Used in: compound_stmt
	: IF test COLON suite star_ELIF ELSE COLON suite {$$=1+$4+$5+$8;}
	| IF test COLON suite star_ELIF {$$=1+$4+$5;}
	;
star_ELIF // Used in: if_stmt, star_ELIF
	: star_ELIF ELIF test COLON suite{$$=$1+$5+1;}
	| %empty{$$=0;}
	;
while_stmt // Used in: compound_stmt
	: WHILE test COLON suite ELSE COLON suite {$$=1+$4+$7;}
	| WHILE test COLON suite {$$=1+$4;}
	;
for_stmt // Used in: compound_stmt
	: FOR exprlist IN testlist COLON suite ELSE COLON suite {$$=1+$6;}
	| FOR exprlist IN testlist COLON suite {$$=1+$6;}
	;
try_stmt // Used in: compound_stmt
	: TRY COLON suite plus_except opt_ELSE opt_FINALLY {$$=$3+$4;}
	| TRY COLON suite FINALLY COLON suite {$$=$3+$6;}
	;
plus_except // Used in: try_stmt, plus_except
	: plus_except except_clause COLON suite {$$=$1+$2+$4;}
...
with_stmt // Used in: compound_stmt
	: WITH with_item star_COMMA_with_item COLON suite {$$=1+$5;}
	;
...
classdef // Used in: decorated, compound_stmt
	:CLASS NAME {methodnumber=0;inclass=1; classname=$2;} LPAR opt_testlist RPAR COLON suite 
	{   int value;
		std::string classout="    C ";
		classout=classout+std::to_string(@1.first_line)+":"+std::to_string((@1.first_column-1));
        //value=round($8/methodnumber);
        size_t n = classfun.size() / 2;
        std::nth_element(classfun.begin(), classfun.begin()+n, classfun.end());
        value=classfun[n];
		//
		if(value<6)classout=classout+" "+$2+" - A ";
		else if(value<10)classout=classout+" "+$2+" - B ";
		else classout=classout+" "+$2+" - C ";
        std::multimap<int,std::string>::iterator it = mymap.find(value);
		mymap.insert ( it,std::pair<int,std::string>(value,classout) );
		methodnumber=0;
		inclass=0;
		classfun.clear();
		}
	| CLASS NAME {methodnumber=0;inclass=1; classname=$2;} COLON suite 
	{
		int value;
		std::string classout="    C ";
		classout=classout+std::to_string(@1.first_line)+":"+std::to_string((@1.first_column-1));
        //value=round($5/methodnumber)+1;
		size_t n = classfun.size() / 2;
        std::nth_element(classfun.begin(), classfun.begin()+n, classfun.end());
        value=classfun[n];
		//
