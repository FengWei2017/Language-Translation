...
TableManager& symboltable = TableManager::getInstance();
%}
...
%token STRING TILDE TRY VBAREQUAL WHILE WITH YIELD  FLOATNUMBER

%type<intNumber> NUMBER pick_unop pick_multop pick_PLUS_MINUS augassign comp_op
%type<id> NAME STRING
%type<fltNumber> FLOATNUMBER
%type<node> atom testlist testlist_comp opt_yield_test pick_yield_expr_testlist_comp power factor term arith_expr 
%type<node>shift_expr and_expr xor_expr expr comparison not_test and_test or_test test opt_test print_stmt star_EQUAL 
%type<node>pick_yield_expr_testlist expr_stmt opt_DOUBLESTAR_NAME  stmt small_stmt star_EQUAL_Right
%type<node> funcdef plus_STRING pick_argument argument  return_stmt flow_stmt simple_stmt star_SEMI_small_stmt 
%type<node> sliceop opt_sliceop opt_test_only  while_stmt break_stmt if_stmt compound_stmt fpdef opt_EQUAL_test 
%type<SL> star_ELIF parameters subscript star_trailer star_slice_trailer trailer slice_trailer subscriptlist star_argument_COMMA arglist 
%type<SL> star_COMMA_test suite varargslist star_COMMA_NAME plus_stmt star_fpdef_COMMA global_stmt opt_arglist
...
funcdef // Used in: decorated, compound_stmt
	: DEF NAME { symboltable.funcLevel += 1; } parameters COLON suite {
		if ($2 != nullptr && $6 != nullptr) {
			...
			Node* func = new FuncNode(n, $4, $6, symboltable.funcLevel);
			if (symboltable.funcLevel == 0) {
				$$ = func;
			} 
			else {
				$$ = nullptr;
			...
			symboltable.setFunctionValue(id, func);
			pool.add(n);
			pool.add(func);
		}

		symboltable.funcLevel -= 1;
		delete[] $2;
	}
	;
...
star_SEMI_small_stmt // Used in: simple_stmt, star_SEMI_small_stmt
	: star_SEMI_small_stmt SEMI small_stmt  { $$ = $3; }
	| %empty                                { $$ = nullptr; }
	;
...
expr_stmt // Used in: small_stmt
	: testlist augassign pick_yield_expr_testlist{
			if ($2 == PLUSEQUAL){
				...
				Node* rhs = new DoubleDivBinaryNode($1, $3);
				$$ = new SuiteNode($1, rhs);
				pool.add(rhs);
				pool.add($$);
			} 
			else if ($2 == PERCENTEQUAL) {
				Node* rhs = new PerBinaryNode($1, $3);
				$$ = new SuiteNode($1, rhs);
				pool.add(rhs);
				pool.add($$);
			}
	...
	: GLOBAL NAME star_COMMA_NAME
		{
			if ($3 == nullptr) {
				...
				pool.addVec($$);
				delete[] $2;
				//std::cout<<"Global_name_star_comma_name"<<std::endl;
			} else {
				Node* n = new IdentNode($2);
				$3->emplace_back(n);
				$$ = $3;
	...
	: star_ELIF ELIF test COLON suite
		{
			if ($1==nullptr) {
				$$ = new std::vector<Node*>;
				Node* else_if_node = new ElifNode($3, $5);
				pool.add(else_if_node);
				$$->emplace_back(else_if_node);
				pool.addVec($$);
			} else {
				Node* else_if_node = new ElifNode($3, $5);
				pool.add(else_if_node);
...
plus_stmt // Used in: suite, plus_stmt
	: plus_stmt stmt
		{
			if ($2 != nullptr) $1->emplace_back($2);
			$$ = $1;
		}
	| stmt
		{
			$$ = new std::vector<Node*>;
			if ($1 != nullptr) $$->emplace_back($1);
			pool.addVec($$);
		}
	;
...
power // Used in: factor
	...
	| atom star_slice_trailer
	 {
		 if ($2 != nullptr && $1 != nullptr) {
				Node* sliceStr = new SliceNode($1, $2);
				pool.add(sliceStr);
				$$ = sliceStr;
		 }
	 }
...
star_slice_trailer // Used in: power, star_trailer 
	: star_slice_trailer slice_trailer { $$ = $2; }
	| slice_trailer {$$ = $1;}
	;
...
slice_trailer // Used in: star_trailer
