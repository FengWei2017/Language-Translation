...
%type<intNumber> NUMBER MINUS PLUS pick_unop TILDE pick_PLUS_MINUS pick_multop augassign 
%type<fltNumber> FLOATNUMBER
%type<id> NAME STRING 
%type<node> atom  power factor  arith_expr test and_test opt_test term pick_yield_expr_testlist star_EQUAL
%type<node> not_test and_expr or_test comparison expr xor_expr shift_expr argument testlist expr_stmt 
%type<node> opt_yield_test pick_yield_expr_testlist_comp yield_expr testlist1 testlist_comp plus_STRING
%type<node> sliceop opt_test_only opt_sliceop
%type<SL> trailer star_trailer  subscriptlist subscript 
...
atom // Used in: power
	...
	| FLOATNUMBER {$$ = new FloatLiteral($1); pool.add($$);}
	| plus_STRING {$$ = $1;}
	;
...
subscript // Used in: subscriptlist, star_COMMA_subscript
	...
	| opt_test_only COLON opt_test_only opt_sliceop { 
		...
		if($3 != NULL) $$->push_back($3);
		if($4 != NULL) $$->push_back($4);
		}
	;
